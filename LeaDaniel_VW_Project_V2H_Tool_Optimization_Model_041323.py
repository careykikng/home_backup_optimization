...
def button(): # function called by Excel VBA macro
    ...
    comp_tool_xl_wb = xw.Book('LeaDaniel_VW_Project_V2H_Computational_Tool_041323.xlsm')
    ...
    if backup_source_config_entered.loc[0,2] > 0: # battery electric vehicle
        ...
        BEV_priority_d = -1 * user_interface.range('I14').value
    ...
    if backup_source_config_entered.loc[6,2] > 0: # plug-in hybrid electric vehicle battery
        ...
        PHEV_surge_kW = PHEV_kW + 0.4*PHEV_kW
        ...
        battery_kWh_all.append(PHEV_battery_kWh)
        ...
        PHEV_priority_d = -1 * user_interface.range('I16').value
    ...
    if backup_source_config_entered.loc[15,2] > 0: # behind the meter storage
        ...
        BTM_priority_d = -1 * user_interface.range('I20').value
    ...
    if backup_source_config_entered.loc[6,2] > 0: # plug-in hybrid electric vehicle ICE
        ...
        PHEV_priority = -1 * user_interface.range('I18').value
    ...
    if backup_source_config_entered.loc[21,2] > 0: # portable generator
        ...
        port_gen_priority = -1 * user_interface.range('I22').value
    ...
    if backup_source_config_entered.loc[25,2] > 0: # standby generator
        ...
        standby_priority = -1 * user_interface.range('I23').value
    ...
    if backup_source_config_entered.loc[14,2] > 0: # photovoltaic generation
        ...
        PV_priority = 10 - (user_interface.range('I19').value/10)
    else:
    ...
    temp_dep_add_starting_watts_all = pd.DataFrame()
    heat_pump_selected = 0

    for i, row in appliances_running_inputs.iterrows(): # save temperature dependent appliances
        if (row[1]) > 0:
            if row[0] == 'Central_air_conditioner_(temperature_dependent)' or row[0] == 'Gas_furnace_(temperature_dependent)' or row[0] == 'Air_source_heat_pump_(temperature_dependent)' or row[0] == 'Space_heater_+_heat_pump_(temperature_dependent)':
                if row[0] == 'Central_air_conditioner_(temperature_dependent)':
                    ...
                    priority_temp_dep_appliances_all.append(row[2]*10)
                    ...
                    temp_dep_AC_add_starting_watts_all = pd.DataFrame(row[5]*row[1], index=range(number_of_time_steps_one_day), columns=range(1))
                    temp_dep_add_starting_watts_all = pd.concat([temp_dep_add_starting_watts_all, temp_dep_AC_add_starting_watts_all], axis=1, ignore_index=True)
                ...
                if row[0] == 'Gas_furnace_(temperature_dependent)':
                    ...
                    temp_dep_GF_add_starting_watts_all = pd.DataFrame(row[5]*row[1], index=range(number_of_time_steps_one_day), columns=range(1))
                    temp_dep_add_starting_watts_all = pd.concat([temp_dep_add_starting_watts_all, temp_dep_GF_add_starting_watts_all], axis=1, ignore_index=True)

                if row[0] == 'Air_source_heat_pump_(temperature_dependent)':
                    heat_pump_selected = 1
                    number_temp_dep_appliances_selected = number_temp_dep_appliances_selected + 1
                    names_temp_dep_appliances_running_all.append(row[0])
                    priority_temp_dep_appliances_all.append(row[2]*10)
                    resistive_heating_starting_watts = HVAC_app_sheet.range('D114').value
                    ...
                    temp_dep_HP_add_starting_watts_all = pd.DataFrame(row[5]*row[1], index=range(number_of_time_steps_one_day), columns=range(1))
                    if user_interface.range('C8').value == "Extreme Winter": # resistive heating starting watts for extreme winter cases
                        for t in range(number_of_time_steps):
                            if float(temp_dep_HP_running_watts_all.iloc[t]) > HVAC_app_sheet.range('D116').value:
                                temp_dep_HP_add_starting_watts_all.iloc[t] = (resistive_heating_starting_watts - temp_dep_HP_running_watts_all.iloc[t])
                    temp_dep_add_starting_watts_all = pd.concat([temp_dep_add_starting_watts_all, temp_dep_HP_add_starting_watts_all], axis=1, ignore_index=True)

                if row[0] == 'Space_heater_+_heat_pump_(temperature_dependent)':
                    if heat_pump_selected == 0:
                        number_temp_dep_appliances_selected = number_temp_dep_appliances_selected + 1
                        names_temp_dep_appliances_running_all.append(row[0])
                        priority_temp_dep_appliances_all.append(row[2]*10)
                        resistive_heating_starting_watts = HVAC_app_sheet.range('E114').value
                        temp_dep_HP_running_watts_all = pd.DataFrame(HVAC_app_sheet.range('E6:E101').value, index=range(number_of_time_steps_one_day), columns=range(1))
                        temp_dep_running_watts_all = pd.concat([temp_dep_running_watts_all, temp_dep_HP_running_watts_all], axis=1, ignore_index=True)
                        temp_dep_HP_add_starting_watts_all = pd.DataFrame(HVAC_app_sheet.range('E108').value, index=range(number_of_time_steps_one_day), columns=range(1))
                        if user_interface.range('C8').value == "Extreme Winter": # resistive heating starting watts for extreme winter cases
                            for t in range(number_of_time_steps):
                                if float(temp_dep_HP_running_watts_all.iloc[t]) > HVAC_app_sheet.range('E116').value:
                                    temp_dep_HP_add_starting_watts_all.iloc[t] = (resistive_heating_starting_watts - temp_dep_HP_running_watts_all.iloc[t])
                        temp_dep_add_starting_watts_all = pd.concat([temp_dep_add_starting_watts_all, temp_dep_HP_add_starting_watts_all], axis=1, ignore_index=True)

                temp_dep_running_watts_all.columns = names_temp_dep_appliances_running_all 
                temp_dep_add_starting_watts_all.to_csv('td_app_starting_watts.csv')

    for i, row in appliances_running_inputs.iterrows(): # save all other appliances
        if (row[1]) > 0:
            if row[0] != 'Central_air_conditioner_(temperature_dependent)' and row[0] != 'Gas_furnace_(temperature_dependent)' and row[0] != 'Air_source_heat_pump_(temperature_dependent)' and row[0] != 'Space_heater_+_heat_pump_(temperature_dependent)': 
                names_appliances_running = row[0]
                names_appliances_running_all.append(names_appliances_running)
                quantity_running = row[1]
                priority_appliances = row[2]*10
    ...
    model.td_app_starting_watts = Param(model.t, model.num_temp_dep_appliances)
    ...
    if number_temp_dep_appliances_selected >0:
        data.load(filename = 'td_app_starting_watts.csv', param = model.td_app_starting_watts, format='array')
    ...
    if number_temp_dep_appliances_selected > 0:
        ...
        def max_starting_watts_td_constraint_rule(model, t, num_temp_dep_appliances):     
            ...
            return (model.starting_watts_td_constr[t,num_temp_dep_appliances] * model.td_app_starting_watts[t,num_temp_dep_appliances]) - sum(model.A[t,j] * model.x[j] for j in j_app) <= sum(backup_surge_kW_all)*1000
    ...
    if count_actual_load_profile > 0:
        def max_starting_watts_actual_home_constraint_rule(model, t, num_actual_home_load_profile):     
            ...
            return (actual_home_add_starting_watts) - sum(model.A[t,j] * model.x[j] for j in j_app) <= sum(backup_surge_kW_all)*1000
...
#    opt.options['mipgap'] = 0.01
    ...
    # get variable 'PV_to_batt' into dataframe
    ...
    calc_values_sheet.range("S6:EC1000").clear_contents()
    calc_values_sheet.range("S2:BW3").clear_contents()
    ...
    if count_actual_load_profile == 0:
        ...
        for d in range(number_temp_dep_appliances_selected):
            ...
            solved_appliance_starting_watts = str_watts_factor_results.mul(temp_dep_add_starting_watts_all.iloc[:,d],axis=0)
            solved_appliance_starting_watts[solved_appliance_starting_watts < 0] = 0

            # if the running watts are 0, then the starting watts are also 0
            for t in range(number_of_time_steps):
                if float(solved_appliance_running_watts.iloc[t]) == 0:
                    solved_appliance_starting_watts.iloc[t] = 0
        ...
        solved_energy_all = pd.DataFrame(all_solved_appliances_running_watts.sum()/4).reset_index(drop=True).transpose()
        percent_supplied_vs_desired = solved_energy_all.sum().sum()/target_energy_all.sum().sum()
        ...
        comp_tool_xl_wb.sheets['Calculation Values']["S3"].options(pd.DataFrame, header=False , index=False, expand='table').value = solved_energy_all
    ...
    if count_actual_load_profile > 0:
        ...
        target_energy_actual_home_load_profile = actual_home_load_profile_day.sum()*time_step_interval
        calc_values_sheet.range('S2').options(index=False, header=False).value = target_energy_actual_home_load_profile
        ...
        solved_energy_actual_home_load_profile = solved_actual_load_profile_results.sum()*time_step_interval
        calc_values_sheet.range('S3').options(index=False, header=False).value = solved_energy_actual_home_load_profile
        calc_values_sheet.range('BY6').options(index=False, header=False).value = 'Actual Home Load Profile [W]'
        calc_values_sheet.range('BY7:BY' + str(number_of_time_steps+6)).options(index=False, header=False).value = actual_home_add_starting_watts
        percent_supplied_vs_desired = solved_energy_actual_home_load_profile/target_energy_actual_home_load_profile
    ...
    if change_total_energy <= 0:
        user_interface.range('Q13').options(index=False, header=False).value = 'Infinite'
    elif change_total_energy > 0:
        user_interface.range('Q13').options(index=False, header=False).value = backup_duration
    
    if count_actual_load_profile == 0:
        if round(percent_supplied_vs_desired, 2) < 1:
            user_interface.range('Q13').options(index=False, header=False).value = percent_supplied_vs_desired
    if count_actual_load_profile > 0:
        user_interface.range('Q13').options(index=False, header=False).value = percent_supplied_vs_desired